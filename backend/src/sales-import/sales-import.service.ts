...
const EXPORT_COLUMNS: { key: keyof SalesDirectoryRowInput; header: string }[] = [
  ...
  { key: 'companyName', header: 'Naziv preduzeÄ‡a' },
  { key: 'city', header: 'Mesto' },
  { key: 'postalCode', header: 'PoĹˇtanski broj' },
  { key: 'address', header: 'Adresa' },
  { key: 'phone', header: 'Telefon' },
  { key: 'legalForm', header: 'Pravni oblik' },
  { key: 'activityCode', header: 'Ĺ ifra delatnosti' },
...
export class SalesImportService {
  ...
  async importFile(tenantId: string, file: Express.Multer.File, userId?: string) {
    ...
    if (!rows) throw new BadRequestException('Supported formats: .xlsx, .xls, .csv');
  ...
  async exportRows(tenantId: string, format: 'csv' | 'xlsx'): Promise<Buffer | string> {
    ...
    if (format === 'csv') {
      ...
      const lines = rows.map((r) =>
        EXPORT_COLUMNS.map(({ key }) => {
          const value = key === 'establishedAt' ? (r.establishedAt ? r.establishedAt.toISOString().slice(0, 10) : '') : ((r as Record<string, unknown>)[key] ?? '');
    ...
    rows.forEach((r) => {
      const rowValues = EXPORT_COLUMNS.map(({ key }) => {
        if (key === 'establishedAt') return r.establishedAt ? r.establishedAt.toISOString().slice(0, 10) : '';
  ...
  private parseCsv(buffer: Buffer): SalesDirectoryRowInput[] {
    ...
    const records = parse(text, { columns: true, skip_empty_lines: true, relax_column_count: true }) as Record<string, string>[];
  ...
  private async parseXlsx(buffer: Buffer): Promise<SalesDirectoryRowInput[]> {
    ...
    ws.eachRow((row, rowNumber) => {
      ...
      if (Object.values(item).some((v) => v != null && String(v).trim() !== '')) out.push(item);
    });
